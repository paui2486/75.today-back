...
if ($_POST['member_id']<> "" && $_POST['board_id']<> "") {
    ...
    if($totalRows_check_member == 1 && $totalRows_check_board == 1 && $totalRows_check_joiner == 0){
        if($_POST['member_id'] != $board_row['m_id']){
            $insertSQL = sprintf("INSERT INTO iwinestand_joiner ( member_id, board_id, create_time)
                                  VALUES (%s,%s,%s)", 
            ...
            $code = 153;
            $status = "could not join the event created by yourself";
        }
        ...
        if($totalRows_check_member == 0 && $totalRows_check_board ==0){
            ...
            $status = " member or board query error.";
        }
